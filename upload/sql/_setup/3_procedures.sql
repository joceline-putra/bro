    ...
    BEGIN
        ...
        BLOCK_B:BEGIN
            ...
            UPDATE temp JOIN (
                ...
                FROM trans 
                WHERE trans_type=1
                AND (CASE WHEN vBRANCH > 0 THEN trans_branch_id=vBRANCH ELSE trans_branch_id > 0 END)
                GROUP BY EXTRACT(YEAR_MONTH FROM `trans_date`)
            ) AS trans ON temp_label=trans.years_month
        ...
        BLOCK_C:BEGIN
            UPDATE temp JOIN (
                ...
                AND (CASE WHEN vBRANCH > 0 THEN journal_item_branch_id=vBRANCH ELSE journal_item_branch_id > 0 END)
    ...
    BEGIN
        ...
        */
        ...
        ELSEIF vACTION = 4 THEN /* Pergerakan Hutang */
            ...
            BLOCK_A:BEGIN /* DONE Get Group of Account */
                ...
                LOOP_1: LOOP
                    ...
                    VALUES (mTRANS_ID,mTRANS_SESSION,CONCAT('Pembelian'),CONCAT(1),mTRANS_DATE,mTRANS_DATE_DUE,fn_time_ago(mTRANS_DATE_DUE),mTRANS_NOTE,mTRANS_NUMBER,mTRANS_TOTAL_DPP,mTRANS_DISCOUNT,mTRANS_TOTAL,mTRANS_TOTAL_PAID,mTRANS_FLAG,mTRANS_paid,mCONTACT_ID,mCONTACT_CODE,mCONTACT_NAME,mBALANCE,mSALES_ID,mUSER_ID);   
                    SET mBALANCE = 0;
                END LOOP LOOP_1;                    
            END BLOCK_A;
        ...
        ELSEIF vACTION = 5 THEN /* Pergerakan Piutang */
            ...
            SELECT * FROM journals_temp ORDER BY trans_date ASC;    
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `sp_report_ar_ap`$$
CREATE PROCEDURE `sp_report_ar_ap`(
        IN vACTION INT(5),
    ...
    BEGIN
        ...
        IF vACTION = 4 THEN /* Pergerakan Hutang */
            DROP TEMPORARY TABLE IF EXISTS journals_temp;
            CREATE TEMPORARY TABLE journals_temp (
                `temp_id` BIGINT NOT NULL AUTO_INCREMENT,
                `type_name` VARCHAR(255),
        ...
        ELSEIF vACTION = 5 THEN /* Pergerakan Piutang */
            ...
            BLOCK_A:BEGIN /* DONE Get Group of Account */
                ...
                DECLARE mACTION_CURSOR CURSOR FOR
                    ...
                    AND `trans_date` < @end_date
                    AND CASE WHEN vACCOUNT_ID > 0 THEN `trans_contact_id`=vACCOUNT_ID ELSE `trans_contact_id` > 0 END
                    AND CASE WHEN vSALES_ID > 0 THEN `trans_sales_id`=vSALES_ID ELSE `trans_sales_id` > 0 END
                    ORDER BY trans_date DESC
                    LIMIT vLIMIT;
                DECLARE CONTINUE HANDLER FOR NOT FOUND SET mFINISHED = 1;   
...
CREATE PROCEDURE `sp_report_stock`(
    ...
    IN vLIMIT_START int(50),
    IN vLIMIT_END int(50)
    )
    BEGIN
        DECLARE mDATE_START_APP VARCHAR(255);
        ...
        /*
            ...
            3 = Kartu Stok vLIMIT_START, vLIMIT_END NOT USED
            4 = Stok di Gudang
        ...
        */
        IF vACTION = 1 THEN /* Stock Gudang */
            ...
            IF vLIMIT_END > 0 THEN SET @limit = CONCAT(' LIMIT ',vLIMIT_START,',',vLIMIT_END); ELSE SET @limit = ''; END IF;
            ...
            SET @SQL_Text = CONCAT('SELECT `trans_items`.`trans_item_product_id` AS `product_id`, `products`.`product_type`, `products`.`product_code`, `products`.`product_name`, `products`.`product_unit`,
                ...
                `category_id`, `category_name`, CONCAT(@COUNT) AS total_data
            FROM `trans_items`
            ...
            GROUP BY `trans_item_product_id` ORDER BY ',vORDER,' ',vDIR,' ',@limit,'');
            PREPARE stmt FROM @SQL_Text;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
        ...
        ELSEIF vACTION = 5 THEN /* Stock Valuation */
            ...
            BLOCK_A:BEGIN /* Group for Product */
                ...
                DECLARE mCURSOR CURSOR FOR
                    ...
                    GROUP BY trans_item_ref
                    HAVING SUM(trans_item_in_qty) - SUM(trans_item_out_qty) > 0
                    ORDER BY trans_item_date ASC LIMIT vLIMIT_START,vLIMIT_END;
                DECLARE CONTINUE HANDLER FOR NOT FOUND SET mFINISHED = 1;
...
CREATE PROCEDURE `sp_user_menu`(vUSER_ID BIGINT(50))
    BEGIN
        ...
        SET @SQL_Text := CONCAT('SELECT * FROM users_menus_temp WHERE user_id=',vUSER_ID,' ORDER BY menu_sorting ASC, id ASC');
