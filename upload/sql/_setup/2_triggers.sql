  ...
  BEGIN
    ...
    */  
    IF mORDER_ITEM_ID IS NOT NULL THEN
      ...
      order_total=order_total_dpp+order_uniq_fee+order_service_fee-order_discount-order_with_dp 
...
DELIMITER $$
DROP TRIGGER IF EXISTS `tr_user_after_delete`$$ 
CREATE TRIGGER `tr_user_after_delete` AFTER DELETE ON `users`
FOR EACH ROW 
BEGIN
  DELETE FROM users_menus WHERE user_menu_user_id=OLD.user_id;
END $$
DELIMITER ;