...
class MY_Controller extends CI_Controller{
    ...
    */
    ...
    public function journal_for_pos($operation,$trans_id,$trans_date,$journal_type,$paid_type,$paid_type_account_id,$branch_id){
        $prepare = "CALL sp_journal_from_pos('$operation',$trans_id,$trans_date,$journal_type,$paid_type,$paid_type_account_id,$branch_id)";
        $query=$this->db->query($prepare);
        $result = $query->result_array();
        mysqli_next_result($this->db->conn_id);
        // $query->next_result();
    ...
    public function report_product_stock($action,$start,$end,$branch_id,$location_id,$product_id,$order,$dir,$search = null,$category = 0,$limit_start = 0,$limit_end = 0){
        $prepare="CALL sp_report_stock($action,'$start','$end',$branch_id,$location_id,$product_id,'$order','$dir','$search',$category,$limit_start,$limit_end)";
        // log_message('debug',$prepare);
        $query=$this->db->query($prepare);
        mysqli_next_result($this->db->conn_id);
        $result = $query->result_array();
    ...
    public function report_ar_ap($action,$start,$end,$branch_id,$account_id,$search,$limit,$sales_id){
        $prepare="CALL sp_report_ar_ap($action,'$start','$end',$branch_id,$account_id,'',$limit,$sales_id)";
        // log_message('debug',$prepare);
        // var_dump($prepare);die;
        $query=$this->db->query($prepare);
        mysqli_next_result($this->db->conn_id);
    ...
    public function file_upload($path, $file, $params = null) { //Binary Upload File
        $return          = new \stdClass();
        $return->status  = 0;
        $return->message = '';
        $return->result  = '';
