<?php defined('BASEPATH') or exit('No direct script access allowed');
    ...
    function upload_file_upload1($path, $file, $compress = null) { // ATTENDANCE.php for $_FILES['file']
        if(!empty($file)){
            ...
            // Make Directory if Not Exists
                ...
                $file_name     = basename($_FILES['upload1']['name']);  
                ...
                if (move_uploaded_file($_FILES['upload1']['tmp_name'], $path . $file_new_name)) {
    ...
    function random_min_max_nominal() {
        return rand(50,300);
    }
    function random_number($length){ # JEH3F2
        $text = '1234567890';
    ...
    function random_session($length){
    ...
    function request_number_for_order($tipe,$branch){
        $ci = &get_instance();
        ...
        $query = $ci->db->query("SELECT MAX(RIGHT(order_number,5)) AS last_number
            FROM orders
            WHERE YEAR(order_date_created)=$tahun
            AND MONTH(order_date_created)=$bulan
            AND order_branch_id=$branch
            AND order_type=$tipe");
        $nomor = "";
        if ($query->num_rows() > 0){
        ...
        $get_init = $ci->db->query("SELECT type_doc FROM `types` WHERE type_for=1 AND type_type=$tipe");
