<script>
    $(document).ready(function () {
            ...
            "columns": [
                {
                    ...
                    render: function (data, meta, row) {
                        ...
                        if (parseFloat(row.contact_receivable_balance) > 0) {
                            ...
                            dsp += '<label class="btn_receivable_payable label label-danger" style="cursor:pointer;padding:0px 2px 2px 2px;"'+
                            ' data-contact-id="' + row.contact_id + '" data-trans-type="2">'+
                            ' Piutang: ' + numberWithCommas(parseFloat(row.contact_receivable_balance)) + "</label></br>";
                        }
                        if (parseFloat(row.contact_receivable_limit) > 0) {
                            // <i class="fas fa-hand-holding-usd"></i> Limit 
                            dsp += '<label class="label label-info" style="padding:0px 2px 2px 2px;">Limit: ' + numberWithCommas(parseFloat(row.contact_receivable_limit)) + "</label>";
                        }
                        return dsp;
                    }
                }, {
        ...
        $(document).on("click", ".btn_receivable_payable", function (e) {
            e.preventDefault();
            e.stopPropagation();
                    ...
                    return $.ajax({
                        ...
                        if (parseInt(s) == 1) {
                            ...
                            if (tt == 1) {
                                var label = 'Supplier';
                                var type = 'Hutang';
                                var title = 'Riwayat Hutang ';
                            } else if (tt == 2) {
                                var label = alias;
                                var type = 'Piutang';
                                var title = 'Riwayat Piutang ';
                            }
                            var dsp = '';
                            dsp += '<table class="table-default">';
                            dsp += '<tr><td><b>' + label + '</b></td><td> : ' + d['contact']['contact_name'] + ' - '+d['contact']['contact_code']+'</td></tr>';
                            dsp += '<tr><td><b>Total (Rp) </b></td><td> : ' + d['total_sisa'] + '</td></tr>';
                            dsp += '</table>';
                            ...
                            dsp += '  <thead>';
                            dsp += '    <th>Nomor</th>';
                            dsp += '    <th class="text-left">Tanggal</th>';
                            dsp += '    <th class="text-right">Total</th>';
                            dsp += '    <th class="text-right">Terbayar</th>';
                            dsp += '    <th class="text-right">Sisa</th>';
                            dsp += '  </thead>';
                            dsp += '  <tbody>';
                            if (parseInt(d['total_records']) > 0) {
                                ...
                                var debt = 0;
                                var crdt = 0;
                                for (var p = 0; p < d['total_records']; p++) {
                                    dsp += '<tr>';
                                    dsp += '    <td><a class="btn-modal-transaksi" href="#" data-id="' + r[p]['trans_id'] + '" data-session="' + r[p]['trans_session'] + '">'+
                                                    '<i class="fas fa-file-alt"></i> ' + r[p]['trans_number'] + '</a>'+
                                                '</td>';
                                    dsp += '    <td>' + moment(r[p]['trans_date']).format("DD-MMM-YYYY, HH:mm") + '</td>';
                                    dsp += '    <td class="text-right"><a class="btn-trans-item-info" href="#" data-id="' + r[p]['trans_id'] + '"'+
                                                    ' data-contact-name="' + d['contact']['contact_name'] + '" data-trans-type="2" data-trans-number="' + r[p]['trans_number'] + '">'+
                                                    '<i class="fas fa-truck-moving"></i> ' + numberWithCommas(parseFloat(r[p]['total_raw']['total'])) + '</a>'+
                                                '</td>';
                                    dsp += '    <td class="text-right"><a class="btn-trans-payment-info" href="#" data-id="' + r[p]['trans_id'] + '"'+
                                                    ' data-contact-name="' + d['contact']['contact_name'] + '" data-trans-type="2" data-trans-number="' + r[p]['trans_number'] + '"'+
                                                    ' data-trans-total="' + numberWithCommas(parseFloat(r[p]['total_raw']['total'])) + '">'+
                                                    '<i class="fas fa-money-bill-alt"></i> ' + numberWithCommas(parseFloat(r[p]['total_raw']['paid'])) + '</a>'+
                                                '</td>';
                                    dsp += '    <td class="text-right">' + numberWithCommas(parseFloat(r[p]['total_raw']['sisa'])) + '</td>';
                                    dsp += '</tr>';

                                    debt    = parseFloat(debt) + parseFloat(r[p]['total_raw']['total']);
                                    balance = parseFloat(crdt) + parseFloat(r[p]['total_raw']['sisa']);                                    
                                    crdt    = parseFloat(crdt) + parseFloat(r[p]['total_raw']['paid']);
                                }
                                dsp += '<tr>';
                                    dsp += '<td colspan="2"><b>Sisa ' + type + ' (Rp)</b></td>';
                                    dsp += '<td class="text-right"><b>' + addCommas(debt) + '</b></td>';
                                    dsp += '<td class="text-right"><b>' + addCommas(crdt) + '</b></td>';                                    
                                    dsp += '<td class="text-right"><b>' + addCommas(balance) + '</b></td>';
                                dsp += '</tr>';
                            } else {
                                dsp += '<tr><td colspan="4">Tidak ada data piutang</td></tr>';
                                dsp += '<tr>';
