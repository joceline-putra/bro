<script>
    $(document).ready(function () {
            ...
            "columns": [
                {'data': 'parent_id',
                    render: function (data, meta, row) {
                        var dsp = '';
                        // dsp += '<a href="#" class="btn_edit_parent" data-id="'+row.parent_id+'" data-name="'+row.parent_name+'" data-icon="'+row.parent_icon+'" data-flag="'+row.parent_flag+'">'+
                        //     '<span class="' + row.parent_icon + '"></span> ' + row.parent_name +
                        // '</a>';
        ...
        $(document).on("click", ".btn_edit_parent", function (e) {
            formMasterSetDisplay(0);
            e.preventDefault();
            var ide = $(this).data("id");
            var iname = $(this).data("name");
            var iicon = $(this).data("icon");  
            var iflag = $(this).data("flag");                                  
            $("#form-master input[name='id_document']").val(ide);
            $("#form-master input[name='nama']").val(iname);
            $("#form-master input[name='icon']").val(iicon);                        
            // if(parseInt(d.result.menu_parent_id) > 0){
            //     $("select[id='group']").append('' +
            //             '<option value="' + d.parent.menu_id + '">' +
            //             d.parent.menu_name +
            ...
            // $("input[name=status][value="+parseInt(d.result.menu_flag)+"]").attr('checked', 'checked');
            $('input[name=status][value='+iflag+']').prop("checked", true).change();                         
        ...
        $(document).on("click","#btn_parent_menu", function(e) {
            e.preventDefault();
            e.stopPropagation();
            $("#modal_parent").modal('show');
            loadParentMenu();
        });
        $(document).on("click", ".btn_update_flag_parent", function (e) {
            e.preventDefault();
            var id = $(this).attr("data-id");
            var flag = $(this).attr("data-flag");
            if (flag == 1) {
            ...
            $.confirm({
                ...
                buttons: {
                    button_1: {
                        ...
                        action: function () {
                            var data = {
                                action: 'update_flag_parent_menu',
                                tipe: identity,
                                id: id,
                                flag: set_flag,
                                nama: nama
        ...
        function loadParentMenu(){
            ...
            form.append('action', 'load_parent_menu');
            form.append('tipe', 4);            
            $.ajax({
                type: "post",
                url: url,
                ...
                success:function(d){
                    ...
                    if(parseInt(s) == 1){
                        notif(s,m);
                        $("#table_parent tbody").html('');
                    
                        let dsp = '';
                        for(let a=0; a < r.length; a++) {
                            let value = r[a];
                            dsp += '<tr>';
                                dsp += '<td><i class="'+value['menu_icon']+'"></i> '+value['menu_name']+'</td>';
                                dsp += '<td>';
                                    if(parseInt(value.menu_flag) < 4){
                                        if (parseInt(value.menu_flag) === 1) {
                                            dsp += '&nbsp;<button class="btn btn_update_flag_parent btn-mini btn-success"';
                                            dsp += 'data-nama="' + value.menu_name + '" data-link="' + value.menu_link + '" data-id="' + value.menu_id + '" data-flag="' + value.menu_flag + '">';
                                            dsp += '<span class="fas fa-check-square primary"></span> Aktif</button>';
                                        } else {
                                            dsp += '&nbsp;<button class="btn btn_update_flag_parent btn-mini btn-danger"';
                                            dsp += 'data-nama="' + value.menu_name + '" data-link="' + value.menu_link + '" data-id="' + value.menu_id + '" data-flag="' + value.menu_flag + '">';
                                            dsp += '<span class="fas fa-times danger"></span> Nonaktif</button>';
                                        
                                            // dsp += '&nbsp;<button class="btn btn_update_flag_parent btn-mini btn-danger"';
                                            // dsp += 'data-nama="' + value.menu_name + '" data-link="' + value.menu_link + '" data-id="' + value.menu_id + '" data-flag="4">';
                                            // dsp += '<span class="fas fa-trash danger"></span> Hapus</button>';
                                        }
                                    }else{
                                        // dsp += '&nbsp;<button class="btn btn_update_flag_parent btn-mini btn-danger"';
                                        // dsp += 'data-nama="' + value.menu_name + '" data-link="' + value.menu_link + '" data-id="' + value.menu_id + '" data-flag="1">';
                                        // dsp += '<span class="fas fa-trash danger"></span> Terhapus</button>';                          
                                    }                              
                                dsp += '</td>';
                            dsp += '</tr>';
