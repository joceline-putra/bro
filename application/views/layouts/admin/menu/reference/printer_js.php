...
$(document).ready(function() {
    ...
    */
    ...
    // document.getElementById("btn_test_direct_printer").addEventListener("click", () => {
    //     const hideFrame = document.createElement("iframe");
    //     hideFrame.onload = setPrint;
    //     hideFrame.style.display = "none"; // hide iframe
    //     hideFrame.src = "print.txt";
    //     document.body.appendChild(hideFrame);
    // });    
    
    $(document).on("click","#btn_test_direct_printer2", function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        var y = screen.height / 2 - 450 / 2;
        var title = 'Print';
        window.open(print_url+'/'+i,'Print','width=700,height=485,left=' + x + ',top=' + y + '').print();
        //window.open(print_url,'_blank');
    ...
    $(document).on("click","#btn_test_bluetooth_printer", function (e) {
        var vale = $("#text").val();
        if(vale.length > 0){
            var set_print_url = url_print + '/struk?trans_id='+vale;
            $.ajax({
                type: "get",
                url: set_print_url,
            ...
            notif(0,'Masukkan ID Transaksi di Teks');
        }
    });
    $(document).on("click","#btn_test_bluetooth_printer_url", function (e) {
        // var id = $(this).attr('data-id');
        var id = $("#text").val();        
        var vale = $("#text").val();
        if(vale.length > 0){        
            printFromUrl('https://jrn.aplikasi.cloud/print_sport_'+id+'.txt');    
        }else{
            notif(0,'Masukkan ID Transaksi di Teks');
        }
    });    
