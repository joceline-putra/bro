                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                ...
                                <td style="text-align:right;">Limit Hutang</td>       
                                <?php if($show_price == 1){?>                             
                                <td style="text-align:right;">Hutang Berjalan</td>
                                <td style="text-align:right;">Hutang Terbayar</td>
                                <td style="text-align:right;">Sisa Hutang</td>
                                <?php } ?>                                                                
                            </tr>
                        </thead>
                    <tbody>
                        ...
                        foreach($content as $v):
                            ...
                            if(floatval($v['contact_payable_balance']) > 0){
                                $balance = number_format($v['contact_payable_balance'],2,'.',',');
                            }else{
                                $balance = '-';
                            }

                            if($show_price == 1){ //Root / Director
                                $running = floatVal($v['contact_payable_running'] > 0) ? number_format($v['contact_payable_running']) : '-';
                                $paid = floatVal($v['contact_payable_paid'] > 0) ? number_format($v['contact_payable_paid']) : '-';
                                $balance = floatVal($v['contact_payable_balance'] > 0) ? number_format($v['contact_payable_balance']) : '-';                          
                            }else{
                                $running = '';
                                $paid = '-';
                                $balance = '-';         
                        ...
                        <tr data-trans-id="<?php echo $v['contact_id'];?>">
                            ...
                            <td style="text-align:right;"><?php echo floatVal($v['contact_payable_limit'] > 0) ? number_format($v['contact_payable_limit']) : '-';?></td>                                                          
                            <?php if($show_price == 1){ ?>
