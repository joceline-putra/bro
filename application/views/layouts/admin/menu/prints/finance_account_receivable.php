      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <p>
          ...
          <div class="col-xs-3 text-right">
            ...
            <?php //echo $header['journal_number'];?>
          </div>                    
        </p>
      </div>
      <div class="col-md-12 col-sm-12 col-xs-12" style="margin-top:4px;">
        <p>
          ...
          <div class="col-xs-6">
            ...
            <br>
            <b>Akun Penerimaan: <br><?php echo '['.$header['account_code'].'] '.$header['account_name'];?></b><br>
          </div>          
        </p>
      </div>      
        ...
        <table class="table table-bordered">
          <thead>
            <tr>
              ...
              <th class="text-left">Atas Faktur Penjualan</th>
          ...
          <tbody>
            ...
            foreach($content as $v){
              ...
              if($v['account_group'] == 1){
                $label = '';
                $item = $v['journal_item_credit'];
                $subtotal_credit = $subtotal_credit + $v['journal_item_credit'];                   
              }else if($v['account_group'] == 4){
                $label = '-';
                $item = $v['journal_item_debit'];
                $subtotal_debit = $subtotal_debit + $v['journal_item_debit'];             
              }  

              echo '<tr>';
                echo '<td>'.$num++.'</td>';
                echo '<td>'.$v['trans']['trans_number'].
                      '<br>'.date("d-M-Y", strtotime($v['trans']['trans_date'])).
                      '<br>'.$v['trans']['contact_name'].
                    '</td>';
                ...
                echo '<td style="text-align:right;">'.$label.number_format($item,2,'.',',').'</td>';                
              echo '</tr>';
            }
              $subtotal = $subtotal_credit - $subtotal_debit;
              //Subtotal
              echo '<tr>';
              ...
              echo '<tr>';
                echo '<td colspan="5"><b>Terbilang: </b>'.$journal_total.'</b></td>';
              echo '</tr>';
