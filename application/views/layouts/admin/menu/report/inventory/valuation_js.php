...
<script>
    $(document).ready(function () {
	    ...
	    $(".nav-tabs").find('li[data-name="report/inventory/product/stock_valuation"]').addClass('active');  
        ...
        var index = $("#table-data").DataTable({
            ...
            "columnDefs": [
                ...
                {"targets": 1, "title": "Nama "+product_alias, "searchable": true, "orderable": true},
                {"targets": 2, "title": "Kategori "+product_alias, "searchable": true, "orderable": true},
                {"targets": 3, "title": "Stok", "searchable": true, "orderable": true},                    
                {"targets": 4, "title": "Satuan", "searchable": true, "orderable": true},
                {"targets": 5, "title": "Harga", "searchable": true, "orderable": true},
                {"targets": 6, "title": "Total Harga", "searchable": true, "orderable": true}                                                
            ],
            // "order": [
            //   [0, 'asc']
            // ],
            "columns": [
                {
                    ...
                    render: function (data, meta, row) {
                        ...
                        var dsp = '';
                        dsp += '<a href="#" class="btn-product-stock" data-product-id="' + row.product_id + '" data-product-name="' + row.product_name + '" data-product-unit="' + row.product_unit + '"><b>' + addCommas(row.qty_balance) + '</b></a>';
                        return dsp;
                    }
                },{
                    'data': 'product_unit', className: 'text-right'
                },{
                    'data': 'qty_in_price', className: 'text-right'
                },{
                    'data': 'qty_in_price_total', className: 'text-right'
                }
            ]
        });
        ...
        $(document).on("click", ".btn-print-all", function () {
            ...
            var print_url = url_print + '/'
                    ...
                    + $("#start").val() + '/'
                    + $("#end").val() + '/'
                    + location + "?product=" + product + "&format=" + format + "&order=" + order + "&dir=" + dir + "&category=" + category + "&location=" + location
                    + "&limit_start="+vstart+"&limit_end="+vend;
