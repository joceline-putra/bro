...
$(document).ready(function() {   
	...
	$(".nav-tabs").find('li[data-name="report/finance/profit_loss"]').addClass('active');    
	...
	var index = $("#table-data").DataTable({
		...
		"ajax": {
			...
			cache: 'false',
			data: function(d) {
				d.action = 'load-report-profit-loss';
				// d.tipe = 1;
		...
		"columns": [
			...
			{'data': 'profit_loss_end',className:'text-right'}      
		...
		createdRow: function(row, data, dataIndex){
			if(data.type_name === 'Saldo Awal'){
				...
				// this.api().cell($('td:eq(1)', row)).data(addCommas(data.profit_loss_debit));            
				// this.api().cell($('td:eq(2)', row)).data(addCommas(data.profit_loss_credit));            
				this.api().cell($('td:eq(1)', row)).data(addCommas(data.profit_loss_end));                 
				...
				if(account_name == '<b>Total</b>'){
					...
					var total_profit_loss_end = '<b>'+addCommas(data.profit_loss_end)+'</b>';            

					if(parseFloat(data.profit_loss_end) < 0){
					var str = data.profit_loss_end;
					var total_profit_loss_end = str.replace('-','');
					total_profit_loss_end = '<b>('+addCommas(total_profit_loss_end)+')</b>';
					}
				...
				// this.api().cell($('td:eq(1)', row)).data(total_profit_loss_debit);
				// this.api().cell($('td:eq(2)', row)).data(total_profit_loss_credit);
				this.api().cell($('td:eq(1)', row)).data(total_profit_loss_end);                                
				}
