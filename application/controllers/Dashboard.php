...
class Dashboard extends MY_Controller{
    ...
    function manage(){
    	...
    	if($request=="top-product"){ // top 10 product
            ...
            $return->status=1;
            $return->message='Loaded';
            $return->branch = $session_branch_id;            
            $return->result= $query->result_array();
    	}else if($request=="total-transaction-day"){ die; // total selling this day
            $type = $this->input->post('type');
            /*$query = $this->db->query("
            ...
            ");
            */
            if(!empty($post['branch']) && ($post['branch'] > 0)){
                $session_branch_id = intval($post['branch']);
            }
            ...
            */
            ...
            // ");
            if(!empty($post['branch']) && ($post['branch'] > 0)){             
                $where_branch = "journal_item_branch_id = ".intval($post['branch']);
            }else{
                $where_branch = "journal_item_branch_id > 0";
            }            
            $prepare = "SELECT
                account_id, account_code, account_name, account_group, account_group_sub,
                  DATE_FORMAT(journal_item_date, '%d %b') AS journal_item_date_short,
                ...
                FROM journals_items
                LEFT JOIN accounts ON journal_item_account_id = account_id
                WHERE $where_branch AND account_locked=1 AND account_group BETWEEN 1 AND 2
                GROUP BY `journal_item_account_id`
                ORDER BY balance DESC
                LIMIT 6
            ";            
            ...
            FROM journals_items
            LEFT JOIN accounts ON journal_item_account_id = account_id
            WHERE $where_branch AND account_group=5 AND account_group_sub=16 AND account_locked=1
            GROUP BY `journal_item_account_id`
            ORDER BY balance DESC";
            // echo $prepare;die;
            $query = $this->db->query($prepare);            
            ...
            $return->message='Loaded';
            $return->result= $query->result_array();
        }else if($request=="finance-list-top-customer"){ // total selling this month
            $type = $this->input->post('type');
            $type = implode(',', $this->input->post('type'));
            $limit = !empty($this->input->post('limit')) ? "LIMIT ".$this->input->post('limit') : '';
