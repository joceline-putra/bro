...
class Inventory extends MY_Controller{
    ...
    var $folder_location = array(
        ...
        '99' => array( //Pemakaian Produk Cabang
            'parent_id' => 41,            
        ...
        if($this->input->post('action')){
            ...
            switch($action){
                ...
                case "load-stock-warehouse":
                    ...
                    $datas = array();
                    $limit_start = !empty($post['start']) ? intval($post['start']) : 0; $limit_end = 10;
                    if($table_reload == 1){
                        $get_datas = $this->report_product_stock(1,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category_id,$limit_start,$limit_end);
                ...
                case "load-stock-moving":
                    ...
                    $datas = array();
                    $limit_start = !empty($post['start']) ? intval($post['start']) : 0; $limit_end = 10;
                    $get_datas = $this->report_product_stock(2,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category_id,$limit_start,$limit_end);
                    $datas_count = count($get_datas);
                    if(isset($get_datas)){ //Data exist
                        foreach($get_datas as $v):
                            $datas[] = array(
                ...
                case "load-stock-valuation":
                    ...
                    if($table_reload == 1){
                        $limit_start = !empty($post['start']) ? intval($post['start']) : 0; $limit_end = 10;
                        $get_datas = $this->report_product_stock(5,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category_id,$limit_start,$limit_end);
                        $datas_count = count($get_datas);
                    }
                    if(isset($get_datas)){ //Data exist
                        foreach($get_datas as $v):
    ...
    function prints_struk($id){ //Print Thermal 58mm Done 
        ...
        if($get_trans){
            ...
            //Content
            $text .= dot_set_wrap_2("Produk", "Qty");
            $text .= dot_set_line('-',$word_wrap_width);
            foreach($get_items as $v):
                $text .= dot_set_wrap_2($v['product_name'], number_format($v['trans_item_out_qty'],0,'',','));            
            endforeach;       

            $text .= "\n";         
            $text .= dot_set_line('-',$word_wrap_width);  
            $text .= dot_set_wrap_2('Cabang',$get_trans['branch_2_name']);
            $text .= dot_set_wrap_2('User',$get_trans['user_username']);
