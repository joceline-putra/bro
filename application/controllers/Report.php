...
class Report extends MY_Controller{
    ...
    function index(){
        ...
        $session_check_buy = !empty($session['user_data']['user_check_price_buy']) ? $session['user_data']['user_check_price_buy'] : 0;
        $session_check_sell = !empty($session['user_data']['user_check_price_sell']) ? $session['user_data']['user_check_price_sell'] : 0;
        $session_user_contact_id= !empty($session['employee']) ? $session['employee']['contact_id'] : 0;  
        ...
        if($this->input->post('action')){
            ...
            switch($action){
                ...
                case "load-report-purchase-buy-account-payable":
                    ...
                    break;  
                case "load-report-purchase-buy-account-payable-dashboard":
                    $columns = array(
                        '0' => 'trans_date',
                        '1' => 'trans_number',
                        '2' => 'trans_total',
                    ...
                    $get_datas = $this->report_ar_ap(4,$date_start,$date_end,$session_branch_id,$contact_id,$search,$set_limit,$sales_id);
                    foreach($get_datas as $k => $v):
                        if(intval($v['total_data']) > 0){
                            $mdatas[] = array(                                 
                                'temp_id' => $v['temp_id'],
                                ...
                                'total_data' => $v['total_data'],
                                'trans_session' => $v['trans_session'],
                                'session_check_buy' => $session_check_buy,
                                'session_check_sell' => $session_check_sell,
                                'trans_by' => array(
                                    'sales_id' => $v['trans_sales_id'],
                                    'user_id' => $v['trans_user_id']
                                )
                            );
                        }
                        $total_data = $v['total_data'];
                    ...
                    $return->params = [
                        ...
                        'sales_id' => $sales_id, 
                        'check_price_buy' => $session_check_buy,
                        'check_price_sell' => $session_check_sell
                    ];                    
                    $return->recordsTotal = $total;
                    $return->recordsFiltered = $total;
                    echo json_encode($return);
                ...
                case "load-report-sales-sell-account-receivable":
                    ...
                    echo json_encode($return);
                    break;  
                case "load-report-sales-sell-account-receivable-dashboard":
                    $columns = array(
                        '0' => 'trans_date',
                        '1' => 'trans_number',
                        '2' => 'trans_total',
                    ...
                    if($session_user_contact_id > 0){
                        $sales_id = intval($session_user_contact_id);
                    }

                    $get_datas = $this->report_ar_ap(5,$date_start,$date_end,$session_branch_id,$contact_id,$search,$set_limit,$sales_id);
                    foreach($get_datas as $k => $v):
                        if(intval($v['total_data']) > 0){
                            $mdatas[] = array(                                 
                                'temp_id' => $v['temp_id'],
    ...
    function pages($request, $identity, $version = null){
        ...
        function report_stock_warehouse($date_start,$date_end,$location){
            ...
            $mdatas = array();
            $limit_start = !empty($this->input->get('limit_start')) ? intval($this->input->get('limit_start')) : 0; 
            $limit_end = !empty($this->input->get('limit_end')) ? intval($this->input->get('limit_end')) : 1000;             
            $mdatas = $this->report_product_stock(1,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category,$limit_start,$limit_end);
        ...
        function report_stock_moving($date_start,$date_end,$location){
            ...
            $limit_start = !empty($this->input->get('limit_start')) ? intval($this->input->get('limit_start')) : 0; 
            $limit_end = !empty($this->input->get('limit_end')) ? intval($this->input->get('limit_end')) : 1000; 
            $mdatas = $this->report_product_stock(2,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category,$limit_start,$limit_end);
        ...
        function report_stock_valuation($date_start,$date_end,$location){
            ...
            $limit_start = !empty($this->input->get('limit_start')) ? intval($this->input->get('limit_start')) : 0; 
            $limit_end = !empty($this->input->get('limit_end')) ? intval($this->input->get('limit_end')) : 1000; 
            $mdatas = $this->report_product_stock(5,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,$category,$limit_start,$limit_end);
        ...
        function report_stock_card($date_start,$date_end,$location){
            ...
            $limit_start = !empty($this->input->get('limit_start')) ? intval($this->input->get('limit_start')) : 0; 
            $limit_end = !empty($this->input->get('limit_end')) ? intval($this->input->get('limit_end')) : 1000;                         
            $mdatas = $this->report_product_stock(3,$date_start,$date_end,$session_branch_id,$location_id,$product_id,$order,$dir,$search,0,$limit_start,$limit_end);
