        ...
        if($this->input->post('action')){
            ...
            switch($action){
                case "load":
                    ...
                    if($identity==4){ //Menu
                        ...
                        if($this->input->post('filter_flag') !== 'ALL'){
                            $params_datatable['child.menu_flag'] = intval($this->input->post('filter_flag'));
                        }else{
                            $params_datatable['child.menu_flag <'] = 5; 
                        }                               
                ...
                case "load_parent_menu":
                    ...
                    $search = null; $limit = null; $start = null; $order  = 'menu_sorting'; $dir = 'ASC';
                    $get_table = $this->Menu_model->get_all_menus($params,$search,$limit,$start,$order,$dir);  
                    $return->status  = 1;
                    $return->message = 'Load Parent';
                    $return->result  = $get_table;                  
                    break;
                case "update_flag_parent_menu":
                    $id = $this->input->post('id');   
                    $nama = $this->input->post('nama');                                
                    $flag = $this->input->post('flag');
                    ...
                    $set_data = $this->Menu_model->update_menu($id,$params);
                    $set_data = $this->Menu_model->update_menu_custom(['menu_parent_id'=>$id],$params);                    
                    if($set_data==true){    
                        //Aktivitas
                        // if($identity == 4){
                        //     $text_code = 'Menu ';
