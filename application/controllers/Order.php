...
class Order extends MY_Controller{
    ...
    function pages($identity){
        ...
        $data['session'] = $this->session->userdata();     
        $data['theme'] = $this->User_model->get_user($data['session']['user_data']['user_id']);
        $data['app_item'] = !empty($session['user_data']['package']) ? $session['user_data']['package']['item'] : []; 
        ...
        */
        ...
        if($identity == 222){ // POS / Point of Sales (Order -> Trans)
            if(!empty($data['app_item']) && ($data['app_item']['support_point_of_sale'] == "Yes")){
    ...
    function manage(){
        ...
        $config_post_to_journal = true; //True = Call SP_JOURNAL_FROM_POS, False = Disabled Function
        ...
        if($this->input->post('action')){
            ...
            switch($action){
                ...
                case "load-order-items": //PO, SO, POS, POS required ref_id (Room, Table)
                    ...
                    if(intval($order_id) > 0){
                        ...
                        if(intval($identity) == 222){ //POS Here
                            ...
                            $get_unsaved_count = $this->Order_model->check_unsaved_order_item_count($identity,$session_user_id,$session_branch_id);
                            if($get_unsaved_count > 0){
                ...
                case "bill-load":
                    ...
                    $return->params = $params;
                    break;
                case "bill-create-backup": //For Sales of POS / order->trans->journal // Penjualan Langsung Lunas
                    $next           = true;
                    $message        = '';
                                ...
                                */
                                ...
                                if($opr){
                                    ...
                                    if($get_trans['trans_id'] > 0){
                                        ...
                                        if($config_post_to_journal==true){
                                            if($identity==222){
                                                $operator = $this->journal_for_pos("create",$trans_id,$journal_date,2,$journal_paid_type,$journal_item_account,$session_branch_id);
                                            } $return->journal_for_pos = 1;
                                        }                                        
                                        // $params = array(
                                        //     'journal_type' => 2,
                                        //     'journal_contact_id' => $get_trans['trans_contact_id'],
    ...
    function prints_orders($id){ // ID = ORDER ID Print Thermal 58mm Done 
        ...
        if($get_order){
            ...
            // $text .= dot_set_wrap_1(date("d/m/Y - H:i:s", strtotime($get_order['order_date'])));    
            // $text .= dot_set_wrap_2('Cashier',$get_trans['contact_name']);

            // $text .= "\n";
            $text .= dot_set_line('-',$word_wrap_width);

            $text .= dot_set_wrap_0($get_order['order_number'],' ','BOTH');
            $text .= dot_set_wrap_0(date("d/m/Y - H:i:s", strtotime($get_order['order_date']))," ","BOTH");  
            if($get_order['order_flag'] == 4){
                $text .= dot_set_wrap_0('BATAL',' ','BOTH'); 
            }          
            $text .= dot_set_line('-',$word_wrap_width);
            ...
            foreach($order_data as $i => $v):
                ...
                foreach($v['order_items'] as $i):
                    ...
                    if(!empty($i['order_item_note'])){
                        $text .= dot_set_wrap_0($i['order_item_note'],' ','RIGHT')."\n";
                    }
                endforeach;            
                $text .= "\n"; 
            endforeach;
            ...
            // $text .= "\n";
            // $text .= dot_set_line('-',$word_wrap_width);
            // $text .= dot_set_wrap_3('SubTotal',':',number_format($get_trans['trans_total'],0,'',','));
            ...
            //Footer
            // $text .= "\n";
            $text .= dot_set_wrap_0("Print On","-","BOTH");
            $text .= dot_set_wrap_0(date("d/m/Y - H:i"),"-","BOTH");            
            // $text .= dot_set_wrap_1("Barang yang sudah di beli tidak dapat ditukar kembali");        
    ...
    function prints_transaction($id){ // ID = TRANS ID Print Thermal 58mm Done 
        ...
        if($get_trans){
            ...
            //Footer
            $text .= "\n";
            $text .= dot_set_wrap_0("Terima kasih atas kunjungannya"," ","BOTH");
            $text .= dot_set_wrap_0("Barang yang sudah di beli tidak dapat ditukar kembali"," ","BOTH");        
