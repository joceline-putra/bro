...
//use PhpOffice\PhpSpreadsheet\Reader\Csv as renderCsv;
            ...
            if($action=='create'){
                ...
                */
                ...
                if(intval($product_id) > 0){
                    ...
                    $datas = array();
                    $limit_start = !empty($post['start']) ? intval($post['start']) : 0; $limit_end = 50;
                    $get_product_stock=$this->report_product_stock(4,'','',$session_branch_id,0,$product_id,'','',$search,0,$limit_start,$limit_end);
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if (isset($_FILES["excel_file"]["name"])) {
                        ...
                        $spreadsheet = $object->load($_FILES['excel_file']['tmp_name']);                        
                        ...
                        //Foreach
                        $numbers = 0;
                        // var_dump(count($sheetData));die;
                        for ($i=0; $i<count($sheetData); $i++) {
                            if(($i>1) && (strlen($sheetData[$i][0]) > 0)){
                                if ($product_type == 1) { //Barang
                                    $product_code = $sheetData[$i][0];
                                    $product_name = $sheetData[$i][1];
                                    $product_unit = $sheetData[$i][2];
                                    $product_price_buy = $sheetData[$i][3];
                                    $product_price_sell = $sheetData[$i][4];
                                    $params_check = [
                                        'product_type' => $product_type,
                                        'product_branch_id' => $session_branch_id,
                                        'product_code' => $product_code
                                ...
                                if ($check_exists) {
                                    ...
                                    $params[] = array(
                                        ...
                                        'product_inventory_account_id' => $get_account_inventory['account_id'],
                                        'product_with_stock' => ($product_type == 1) ? 1 : 0 
                                    );
                                }
                                $numbers++;       
                            }         
