...
class Order_model extends CI_Model {
    ...
    function get_all_orders_and_orders_items($params = null, $search = null, $limit = null, $start = null, $order = null, $dir = null) {
        $this->db->select("orders.*");
        $this->db->select("orders_items.*");
    ...
    function get_all_orders_and_orders_items_count($params,$search){
        $this->db->from('orders');   
        $this->db->join('orders_items','order_item_order_id=order_id','left');
        $this->db->join('products','order_item_product_id=product_id','left');
        $this->set_params($params);         
    ...
    */
    ...
    function get_order_item_custom($where){
        $this->db->select("orders.*");
        $this->db->select("orders_items.*");
        $this->db->select("products.*");
        $this->db->select("order_id AS order_id, DATE_FORMAT(`order_date`,'%d-%b-%y, %H:%i') AS order_date_format, IFNULL(order_total,0) AS order_grand_total");
        $this->db->join('orders','order_item_order_id=order_id','left');
        $this->db->join('products','order_item_product_id=product_id','left');        
        return $this->db->where($where)->get('orders_items')->row_array();
    }
    /* function to update order */
